...
当前系统已经具备稳定的 USB 引导、机器人系统固件下载与校验、7×13 LED 点阵显示、SC7A20HTR 采样及角度输出、五路串口基础初始化。Python 解释器、电源键业务、用户按键 API、蓝牙应用协议、外接电机／传感器协议和音频协议尚未实现。
...
flowchart LR
    ...
    MCU[APM32E103RET6]
    BOOT[USB 引导]
    FW[机器人系统固件<br/>当前为演示镜像]
    RAM[RAM<br/>后续用户程序临时驻留]
...
机器人自身 USB 与 DAPLink 是两条独立链路。机器人 USB 当前用于系统固件升级、运行期命令和传感器日志，未来也用于把用户程序送入 RAM；DAPLink 经 SWD 用于首次写入引导、Flash 读回、寄存器检查和源码级调试。
...
| `tools/build.py --demo` | 定义 `KLQ_DEMO_APP`；`0x08008000` | 当前机器人系统固件演示镜像：USB CDC、TM1640、五路 UART、SC7A20HTR 和角度日志 |

引导构建会排除 `peripherals.c` 和 `sc7a20.c`，因此保持“只驱动 USB 与显示”的最小边界。当前演示镜像是机器人系统固件的前身，不是用户程序；它不编译 Flash 下载命令，只保留 INFO、ECHO、DISPLAY 和 RESET 等运行期诊断命令。
...
flowchart TD
    ...
    INIT[系统固件：初始化 UART1～5 和 SC7A20HTR]
...
当前每次硬件复位都进入引导，不自动运行系统固件。主机只有发送 RUN 命令后才跳转至有效系统固件。这保证系统固件异常时仍可通过复位回到下载入口。最终产品的自动启动和按键进入引导策略尚未实现。
...
flowchart LR
    ...
    LINK[USB／蓝牙用户程序下载协议<br/>尚未实现]
    RAM[MCU RAM<br/>源码或编译后表示／校验信息／运行状态]
    VM[系统固件中的 Python 解释器]
...
用户程序下载完成后必须先在 RAM 中完成长度和完整性校验，再标记为可运行。下载失败或中断时清除 RAM 中的有效状态，不执行残缺内容。替换程序时停止当前程序，再用新内容替换 RAM 中的程序缓冲。RAM 容量分配和最大用户程序长度需要在选择解释器后测量确定。
...
| INFO / ECHO | 支持 | 支持 |
| DISPLAY | 支持 | 支持 |
| BEGIN / DATA / END（系统固件升级） | 支持 | 不支持 |
| RUN | 支持 | 不支持 |
| RESET | 支持 | 支持，返回引导 |
| 文本日志 | 不输出 | 约 10 Hz 输出三轴和 Roll/Pitch |

当前 BEGIN、DATA、END 和 RUN 只操作 MCU 内部 Flash 中的机器人系统固件。未来的用户程序下载必须由正在运行的系统固件接收并写入 RAM，应使用与系统固件升级明确区分的命令和状态机；蓝牙与 USB 可共用这一用户程序应用层协议。

当前演示系统固件在同一个 CDC 字节流中同时承载 KLQ1 二进制命令和 ASCII 传感器日志。主机协议客户端会搜索 `KLQ1` 魔数重新同步，串口监视器按行显示文本。这一方式适合板级调试；正式应用建议把日志封装为协议消息或增加明确的通道管理，避免高日志量影响控制报文。
...
| 路径 | 当前职责 |
| --- | --- |
| `bootloader/board.c` | 最早电源保持、时钟、SysTick、USB 断开和系统固件跳转 |
| `bootloader/tm1640.c` | 7×13 点阵映射、亮度命令和引导图标 |
| `bootloader/usb_serial.c` | Geehy USB Device 库适配、CDC 收发和缓冲 |
| `bootloader/protocol.c` | KLQ1 帧解析、命令处理、CRC 和系统固件更新状态机 |
...
用户程序下载至 RAM、外接串口设备通信、ECB02C 数据传输、音频播放、电池采样、按键交互、整板最终业务固件冷启动和 Python 代码执行不在当前已验证范围内。

## 11. 后续机器人系统固件架构

在保持引导分区和 KLQ1 更新协议稳定的前提下，下一阶段机器人系统固件建议从当前演示循环拆成以下模块：
...
Python 层只暴露有限且稳定的 KLQ API，首要范围是用户按键检测、板载或串口传感器数据读取，以及串口电机控制。用户代码不直接操作 PC5、系统菜单、内部／外部 Flash 或任意 MCU 寄存器。SC7A20HTR 和外接端口均通过系统服务访问，以便停止、超时和错误处理保持一致；显示和音频是否开放给用户程序后续再定。
