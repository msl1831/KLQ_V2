...
当前系统已经具备稳定的 USB 引导、机器人系统固件下载与校验、有效固件自动启动、7×13 LED 点阵显示状态机、16 KiB RAM 用户程序接收与精简 Python 解释执行、PC3 用户键、PC4 短按运行/停止、SC7A20HTR 采样，以及 UART1～3 KLQ1 自动发现与 CS100A 超声服务。PC4 长按关机、蓝牙应用协议、外接电机/红外设备协议和音频协议尚未实现。
...
| 电源键 | PC4 | 短按运行／停止已接入；长按关机与菜单确认待实现 |
...
上电和没有引导请求的普通复位会自动运行通过元数据、向量和 CRC 校验的系统固件。系统固件处理 USB `reset` 时在 RAM 顶部 `0x2001FFF8` 写入魔数及反码，然后软复位；引导消费并立即清除标记后停留在下载界面。系统固件无效时也停留在引导。实体按键强制进入引导尚未实现，恢复入口和键序待定。
...
开机动画由中心光点逐级扩展、收拢，再显示闭眼笑脸并睁眼进入待机。待机笑脸每隔约 4.5～8.6 秒做一次双眨眼，间隔由 16 位 LFSR 变化；离开待机后不执行眨眼。下载期间显示向下箭头，校验成功显示对勾 800 ms；用户程序进入运行状态时显示运行三角形。只有运行状态接受用户图案，停止或正常结束后清除用户图案并恢复待机笑脸。错误 CRC 显示叉号 800 ms 后返回待机。
...
| `bootloader/power_key.c` | PC4 上拉输入、25 ms 去抖、短按运行／停止及上电按压抑制 |
...
精简 Python 已完成构建和实板执行验证。除 CS100A 外的外接设备类型、ECB02C 数据传输、音频播放、电池采样及 PC4 长按关机仍不在已验证范围内。PC4 短按运行／停止已接入；按键有效电平和动作由实物按键测试确认。PC3 用户键驱动与 SC7 倾斜条件已实现，15° 阈值和前后左右方向仍需实物交互标定。电机和红外仅有稳定 Python API 与底层接入点，取得真实设备协议前会明确返回“不支持”。
