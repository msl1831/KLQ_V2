 ...
 * @retval      None
 */
#if defined(APM32F0xx_USB)
void USB_IRQHandler(void)

#elif defined(APM32F10x_USB) || defined(APM32E10x_USB)
#if USB_SELECT == USB1
...
#endif
{
    ...
    if(USBD_ReadIntFlag(USB_INT_PMAOU))
    {
        USBD_ClearIntFlag(USB_INT_PMAOU);
    }
#endif
 ...
 * @retval      None
 */
/* Modified for KLQ 2026-09-14 under the Geehy package license.
 * Do not reset endpoints or spin waiting for a bus RESET during suspend.
 * The loader keeps CPU/GPIO clocks running for power hold and display. */
static void USBD_SuspendIsrHandler(void)
{
...
#if defined(APM32F10x_USB) || defined(APM32E10x_USB)
/*!
