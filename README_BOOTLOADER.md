...
上电或普通复位时，引导先校验系统固件；镜像有效则自动启动，系统固件播放开机动画并进入待机。系统固件收到 USB `reset` 后，会在 RAM 顶部写入带反码校验的一次性引导请求再软复位；引导消费并清除该请求后停留在下载界面。系统固件无效时也停留在引导。实体按键强制进入引导仍未实现，恢复入口和键序待定。
