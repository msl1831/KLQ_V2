...
本工程面向 APM32E103RET6。引导仅初始化电源保持、PC4 长按开机判断、时钟、TM1640 点阵和 USB，不初始化蓝牙、用户传感器、电机或外部 Flash。

当前已烧录的主线引导为 9828 字节。机器人系统固件的 USB 下载、校验、自动启动、返回引导及连续重连实测通过，见 [验证记录](docs/verification_20260915_ui_ram.md)和[电源键回归记录](docs/verification_20260923_power_on_off.md)。当前串口为 COM123，重新接入时编号可能改变。
...
| PC4 | 上拉、按下为低；冷上电持续按住 1.5 秒才继续启动，提前松开则 PC5 拉低 |
...
进入引导显示下载箭头。系统固件下载校验通过后显示对勾，镜像校验失败显示叉号。引导实现冷上电长按门槛，停留引导时也能长按 1.5 秒关机；运行中的长按关机由系统固件处理。引导没有按键菜单。

冷上电先确认 PC4 持续按住 1.5 秒；软件复位不重复要求按键。随后引导校验系统固件；镜像有效则自动启动，系统固件播放开机动画并进入待机。系统固件收到 USB `reset` 后，会在 RAM 顶部写入带反码校验的一次性引导请求再软复位；引导消费并清除该请求后停留在下载界面。系统固件无效时也停留在引导。实体按键强制进入引导仍未实现，恢复入口和键序待定。
...
这是单系统固件槽方案，开始替换后不保留旧版本回滚副本。引导不通过 USB 更新自身，也不修改保护选项字节。Python 用户程序由系统固件接收、校验并存入 16 KiB RAM 缓冲，由精简解释器执行，复位或断电后失效。
...
上位机下载 Python 源码请直接使用[上位机 USB 通信协议](docs/host_usb_protocol_v0.1.md)。以下为引导与系统固件共用的帧格式概览。
